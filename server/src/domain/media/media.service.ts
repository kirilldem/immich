...
export class MediaService {
  ...
  async handleGenerateJpegThumbnail({ id }: IEntityJob): Promise<JobStatus> {
    ...
    await this.assetRepository.updateAll([asset.id], { resizePath });
    return JobStatus.SUCCESS;
  }
  ...
  async handleGenerateWebpThumbnail({ id }: IEntityJob): Promise<JobStatus> {
    ...
    await this.assetRepository.updateAll([asset.id], { webpPath });
    return JobStatus.SUCCESS;
  }
  ...
  async handleGenerateThumbhashThumbnail({ id }: IEntityJob): Promise<JobStatus> {
    ...
    await this.assetRepository.updateAll([asset.id], { thumbhash });
  ...
  async handleVideoConversion({ id }: IEntityJob): Promise<JobStatus> {
    ...
    if (target === TranscodeTarget.NONE) {
      if (asset.encodedVideoPath) {
        ...
        await this.assetRepository.updateAll([asset.id], { encodedVideoPath: null });
      }
    ...
    await this.assetRepository.updateAll([asset.id], { encodedVideoPath: output });
