        ...
        onUnlink: (path) => {
          const handler = async () => {
            ...
            if (asset && matcher(path)) {
              await this.assetRepository.updateAll([asset.id], { isOffline: true });
            }
  ...
  async handleAssetRefresh(job: ILibraryFileJob): Promise<JobStatus> {
    ...
    try {
      ...
      if (existingAssetEntity) {
        ...
        await this.assetRepository.updateAll([existingAssetEntity.id], { isOffline: true });
        return JobStatus.SUCCESS;
      } else {
    ...
    if (stats && existingAssetEntity?.isOffline) {
      ...
      await this.assetRepository.updateAll([existingAssetEntity.id], { isOffline: false });
      doRefresh = true;
    }
